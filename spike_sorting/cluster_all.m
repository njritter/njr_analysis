...
for i = 1:length(expnames),
    ...
    for j = 1:length(namerefs), % for all 32 channels
        ...
        [times,dirends,waves,~,~,EpochNames,~] = read_spikes_intan(ds,name,ref); % read spikes for chan j
        
        [clusterids,CI] = cluster_spikewaves(waves,ref,EpochNames{1},EpochNames{end});
        
        write_spikes(ds,clusterids,CI,name,ref,EpochNames,dirends,times);
